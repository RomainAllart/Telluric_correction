...
def save_files(path,instrument,flux_corr,flux_err_corr,telluric,result_fit,molecules,save_type):
    ...
    if save_type == 'DRS':
        create_folder('../Output/'+Vdrs+'/'+date+'/DRS')
        os.system('cp '+path+' ../Output/'+Vdrs+'/'+date+'/DRS/'+path.split('/')[-1][:-5]+'_CORR.fits')
        file = '../Output/'+Vdrs+'/'+date+'/DRS/'+path.split('/')[-1][:-5]+'_CORR.fits'        
        hdr = fits.Header()
        ...
        create_folder('../Output/'+Vdrs+'/'+date+'/DAS')
        os.system('cp '+path+' ../Output/'+Vdrs+'/'+date+'/DAS/'+path.split('/')[-1][:-5]+'_CORR.fits')
        file = '../Output/'+Vdrs+'/'+date+'/DAS/'+path.split('/')[-1][:-5]+'_CORR.fits'        
        hdr = fits.Header()
...
parser.add_argument("drs_v",type=str,help="drs version")
args = parser.parse_args()

import glob

#night='2022-06-16'
files_nirps_drs=glob.glob('../../'+args.drs_v+'/reduced/'+args.night+'/r.NIRPS.*S2D_BLAZE_A.fits')
#files_nirps_drs=glob.glob('../../mounzer/ATC_Tests/*S2D_BLAZE_A.fits')
...
if __name__ == "__main__":
    multiprocessing_ATC(files_nirps_drs,['NIRPS_drs',['H2O','O2','CO2','CH4']],nthreads=6)
